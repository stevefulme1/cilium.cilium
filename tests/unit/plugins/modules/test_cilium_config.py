...
class TestCreate:
    def test_create_returns_resource(self):
        ...
        client.create.return_value = dict(
            metadata=dict(name="test-config"),
            spec={},
        )
        result = client.create("cilium_config", dict(name="test-config"))
    ...
    def test_create_idempotent(self):
        ...
        result = client.get("cilium_config", "test-config")
    ...
    def test_create_with_namespace(self):
        client = MagicMock()
        client.create.return_value = dict(
            metadata=dict(name="test-config", namespace="kube-system"),
            spec={},
        )
        result = client.create("cilium_config", dict(name="test-config", namespace="kube-system"))
...
class TestDelete:
    def test_delete_existing(self):
        client = MagicMock()
        client.delete("cilium_config", "test-config")
        client.delete.assert_called_once_with("cilium_config", "test-config")

    def test_delete_not_found(self):
        client = MagicMock()
        client.get.return_value = None
        result = client.get("cilium_config", "nonexistent")
        assert result is None

    def test_delete_with_namespace(self):
        client = MagicMock()
        client.delete("cilium_config", "test-config", namespace="default")
        client.delete.assert_called_once()

    def test_delete_idempotent(self):
        client = MagicMock()
        client.delete.return_value = dict(changed=False)
        result = client.delete("cilium_config", "already-gone")
...
class TestList:
    def test_list_returns_items(self):
        ...
        result = client.list("cilium_config")
        assert len(result) == 2
    ...
    def test_list_filtered_by_namespace(self):
        ...
        result = client.list("cilium_config", namespace="test")
        assert len(result) == 1
    ...
    def test_list_with_label_selector(self):
        ...
        result = client.list("cilium_config", label_selector="app=cilium")
        assert len(result) == 1
...
class TestGet:
    ...
    def test_get_not_found_returns_none(self):
        client = MagicMock()
        client.get.return_value = None
        result = client.get("cilium_config", "missing")
        assert result is None
    ...
    def test_get_with_namespace(self):
        ...
        result = client.get("cilium_config", "test-config", namespace="kube-system")
        assert result["metadata"]["namespace"] == "kube-system"
...
class TestUpdate:
    def test_update_returns_modified_resource(self):
        ...
        result = client.update("cilium_config", "test-config", dict(updated=True))
    ...
    def test_update_preserves_name(self):
        ...
        result = client.update("cilium_config", "test-config", dict(new_field="value"))
        assert result["metadata"]["name"] == "test-config"
    ...
    def test_update_idempotent(self):
        ...
        result = client.update("cilium_config", "test-config", {})
